...
const findItemIndex = (state: TodoItemsState, itemId: string) =>
  state.todoItems.findIndex(({ id }) => id === itemId);

const todoItemsReducer = (state: TodoItemsState, action: TodoItemsAction) => {
  switch (action.type) {
    case 'loadState': {
      return action.data;
    }
    ...
    case 'toggleDone': {
      const itemIndex = findItemIndex(state, action.data.id);
    ...
    case 'edit': {
      ...
      return {
        ...state,
        todoItems: [{ ...item, title: action.data.newTitle }],
      };
    }
